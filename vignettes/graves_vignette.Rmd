...
data <- graves %>% filter(g_date_birth != "0001-01-01", 1) %>% filter(g_date_birth != "1900-01-01", 1) %>% 
  select(g_date_birth) %>% 
...
plot_ly(data = data, x = ~g_date_birth, y = ~n, type = "bar") %>%
layout(xaxis = list(range = c("1800","2018")))

data2 <- graves %>%filter(g_date_death != "0001-01-01", 1) %>% 
...
plot_ly(data = data2, x = ~g_date_death, y = ~n, type = "bar") %>% 
  layout(xaxis = list(range = c("1800","2018")))


data3 <- graves %>% filter(g_date_burial != "0001-01-01", 1) %>%
...
plot_ly(data = data3, x = ~g_date_burial, y = ~n, type = "bar") %>%
 layout(xaxis = list(range = c("1800","2018")))
...
# datatime analysis cemetery no 1 "Miłostowo"
```{r}
choice_cm_1 <- graves %>%
  #sqldf("select * from graves where g_date_death between 2000-01-01 and 2010-01-01") %>% 
  select(c(g_date_death,cm_id))

clean_cm_1 <- choice_cm_1 %>% 
  count(cm_id) %>% 
  filter(cm_id == 1) %>% 
  arrange(n)

plot_ly(data = clean_cm_1, x = ~g_date_death, y = ~n, type = "scatter", mode = "markers")

plot(clean_cm_1$n, main = "zgony")
...
# datatime analysis cemetery no 2 "Junikowo"
```{r}
