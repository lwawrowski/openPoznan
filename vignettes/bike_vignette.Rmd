...
date <- data.frame(date =example$date,n = 1066 - example$n )
head(date)
```
#Mediana
```{r}
...
```{r}
  ...
  ggplot(data = date, aes(n)) + geom_histogram(binwidth = 3)


```
#Mediana wypożyczeń rowerów z kazdej godziny
```{r}
date <- cbind(date,data.frame(hours =as.numeric(substring(date$date,12,13))))
